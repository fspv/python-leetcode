...
class TestSubmissionResult(BaseSubmissionResult):
    ...
    def code_answer(self, code_answer):
        ...
        if code_answer is None:
            raise ValueError("Invalid value for `code_answer`, must not be `None`")  # noqa: E501
    ...
    def expected_status_code(self, expected_status_code):
        ...
        if expected_status_code not in allowed_values:
            raise ValueError(
                "Invalid value for `expected_status_code` ({0}), must be one of {1}"  # noqa: E501
                .format(expected_status_code, allowed_values)
            )
